...
const SelectWarehouseComp = React.forwardRef(({
    onSelected = () => { },
    items = 0,
    WareHouseId = 0,
    AreaId = 0,
    ...
    const WH_spWareHouse_Select = async () => {
        ...
        try {
            ...
            if (list.length > 0) {
                ...
                if (items != 0 && items != -1) {
                    ...
                    if(ar !== undefined)
                        setValueS({ value: ar.WareHouseId, name: ar.WareHouseName, label: ar.WareHouseCode + '-' + ar.WareHouseName });
                } // Active
                setData(dataSelect)
            } else {
                setData([]);
            ...
            console.log(error, "WH_spWareHouse_Select")
        }
    }

    useEffect(() => {
        WH_spWareHouse_Select()
    }, [WareHouseId, AreaId]);
...
export const SelectWarehouse = React.memo(SelectWarehouseComp)