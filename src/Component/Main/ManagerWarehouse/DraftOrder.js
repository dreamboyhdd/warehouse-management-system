...
export const DraftOrder = () => {
        ...
        if (e.charCode == 13) {
            ...
            if (Code != "") {
                ...
                if (Detal_tmp.length > 0) { // Scan in seri
                    let Tmp = ExportData_Detail.find(e => e.CodeSeriIme === Code && e.IsCheckProduct === 0 && e.PackageNumber != e.NumberScan)
                   
                    console.log(Tmp,'tessttt')
                    if (Tmp != undefined) {
                        CheckScanProduct(Code, 0)
                    } else {
                        let Data_tmp = ExportData_Detail.filter(e => e.ProductCode === Code && [0, 1].indexOf(e.IsCheckProduct) != -1 && e.PackageNumber != e.NumberScan);
                        if (Data_tmp.length > 0) {
                            CheckScanProduct(Code, 1)
                        } else {
    ...
    const CheckScanProduct = (Code, key) => {
        ...
        let Tmp = []
        if (key === 0) {
            Tmp = ExportData_Detail.find(e => e.CodeSeriIme === Code && e.IsCheckProduct === 0 && e.PackageNumber != e.NumberScan)
            Tmp.NumberScan = Tmp.UnitNumberConversion;
            Tmp.NumberItem = Tmp.UnitNumberConversion;
        }
        else { // Scan theo sản phẩm
            Tmp = ExportData_Detail.filter(e => e.ProductCode === Code && [0, 1].indexOf(e.IsCheckProduct) != -1 && e.PackageNumber != e.NumberScan)[0]
        ...
        if (ExportData_Tmp.NumberScan == ExportData_Tmp.NumberConversion) {// Nếu số lần scan = số phẩm order thì đánh dấu order thành công
            ...
            ExportData.find(e => e.ExportCode == Tmp.ExportCode).IsCheckProduct = 2 // Đánh dấu order đã finish
        }
        ...
        if (ExportData_Tmp.NumberConversion >= 2)
                                        ...
                                        <div className='row'>
                                            <div className="col-sm-6">
                                                <div className="dropdown">
                                                    <h3 className="dropbtn margin-5"><i className="fa-solid fa-plus"></i> DRAFT ORDER</h3>
                                                </div>
                                            </div>
                                            <AudioTrue
