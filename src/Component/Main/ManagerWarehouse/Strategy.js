...
import $ from "jquery";
import { SelectArea, SettingColumn, SelectAccount, SelectCustomer, SelectWarehouseContract, SelectWarehouse, SelectVehicle, DataTable, SelectProvider, SelectCurator } from '../../../Common';
import { mainAction } from '../../../Redux/Actions'
import { Alertwarning, ConfirmAlert, Alertsuccess, Alerterror, GetDataFromLogin, FormatDateJson, FirstOrLastDayinMonth, FormatNumber, ExportExcel, DateDiff } from "../../../Utils";
...
export const Strategy = () => {
    ...
    useEffect(() => {
        ...
        if (dataarray.keys === 'check') {
            ...
            WH_spWareHouse_Strategy_Confirm(dataarray._row.value)
        }
    ...
    const WH_spWareHouse_Strategy_Confirm = async (Id) => {
        try {
            const pr = {
            ...
            const params = {
                ...
                func: 'WH_spWareHouse_Strategy_Confirm',
            };
    ...
    const handleCheckPoCode = async (Code) => {
        try {
            console.log(Code)
        } catch (error) {
    ...
    const StrategySave = async () => {
        ...
        // if (Function.Id != 2 && POcode === '') {
        //     Alertwarning("Vui lòng nhập mã PO!")
        //     POcodeRef.current.focus();
        //     return
        // }
        ...
        // if (Provider.value === -1 || Provider.value === 0) {
        //     Alertwarning("Vui lòng chọn nhà cung cấp!")
        //     ProviderRef.current.focus();
        //     return
        // }
        // if (Curator.value === -1 || Curator.value === 0) {
        //     Alertwarning("Vui lòng chọn người phụ trách!")
        //     CuratorRef.current.focus();
        //     return
        // }
        ...
        DataOK.forEach((i, k) => {
            if (i.ValuePackaging !== undefined && i.sttcheck !== false) {
                ar.push({
                    ...
                    ProductPackagingName: i.ValuePackaging.label, // Tên đơn vị tính
                    NumberConversion: i.ValuePackaging.NumberConversion, // Số lượng quy đổi ( 10 cái = 1 hộp)
                    WareHouseId: 0,
                    WareHouseAreaId: 0,
        ...
        if (ar.length == 0 && DataOK.length > 0) {
            Alertwarning("Vui lòng nhập sản phẩm!")
            return
        }
        if (ar.length == 0) {
        ...
        try {
            ...
            if (list[0].Status === 'ExistSeri') {
                list.map(e => {
                    Alertwarning(`Mã ${e.ProductCode} có số Serial/Imei ${e.CodeSeriIme} đã tồn tại`)
                })
            }
            if (list[0].Status == 'OK') {
    ...
    const dropdown = (
        ...
        >
            <a id={'1'} _funcsave={'WH_spWareHouse_Import_Save_V2'}>chiến lược nhập kho</a>
            <a id={'2'} _funcsave={'WH_spWareHouse_Export_Save'}>chiến lược xuất kho</a>
            <a id={'3'} _funcsave={'WH_spWareHouse_Transport_Save'}>chiến lược chuyển kho</a>
            <a id={'4'} _funcsave={'WH_spWareHouse_Check_Save'} >chiến lược kiểm kho</a>
        </div>
    )
                            ...
                            <div className="card-body group__product card__info">
                                ...
                                <div className="row col-md-6">
                                    <div className="card__h150w210 BG-9DDFD3">
                                        <p className="">nhóm sản phẩm</p>
                                        <p>{CountGroup}</p>
                                    </div>
                                    <div className="boxtotal">
                                        <p className="">tổng mã sản phẩm</p>
                                    ...
                                    <div className="card-body-form">
                                        <div className="row col-md-12">
                                            ...
                                            <div className="row col-md-12 linetop">
                                                <div className="col-md-4 margin-left-5">
                                            ...
                                            <div className="row col-md-12 linetop">
                                                ...
                                                <div className="table-responsive" >
                                                    <table className="table table-head-fixed text-nowrap margin-bottom-80 table__detail__with___btn">
                                                        <thead>
                                                            <tr>
                                                                <th>Tuỳ chọn</th>
                                                                <th>STT</th>
                                                                        ...
                                                                        <tr key={k}>
                                                                            ...
                                                                            {Function.Id !== '3' ?
                                                                                <td className="min-width200">
                                                                                    <div>
                                                                                        <input type="text" key={item.ProductId} min={0} value={item.Lotnumber} onChange={e => handleChangeSerial((k + 1), e.target.value)} className="form-control top-2" />
                                                                                    </div>
                                                                                </td>
                                                                                : ""}
                                                                            {Function.Id !== '3' ?
                                                                            ...
                                                                            {Function.Id !== '4' ?
                                                                                <td className="min-width200">
                                                                                    <div className="form-group">
                                                                                        <input type="number" key={item.ProductId} min={0} value={item.PackageNumber} onChange={e => WeightExchange((k + 1), e.target.value)} className="form-control top-2" />
                                                                                    </div>
                                                                                </td> : ""
                                                                            }
                                                                            {Function.Id != '1' ?
                                                                                <td className="min-width200">
                                                                                    <div className="form-group">
                                                                                        <input type="number" key={item.ProductId} min={0} readOnly value={item.NumberTotal} className="form-control top-2 text-center" />
                                                                                    </div>
                                                                                </td> : ""
                                                                            }
                                                                            {Function.Id !== '4' ?
                                                                                <td className="min-width200">
                                                                                    <div className="form-group">
                                                                                        <input type="text" key={item.ProductId} readOnly value={item.TotalWeight} className="form-control top-2 text-center" />
                                                                                    </div>
                                                                                </td> : ""
                                                                            }
                                                                            {Function.Id !== '3' ?
